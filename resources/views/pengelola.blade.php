...
<main class="h-full pb-16 overflow-y-auto">
    ...
    <div class="flex justify-between w-full">
        ...
        <div class="flex px-0 my-6 space-x-3">
            ...
            @endauth
            @include('components.cetak-btn', ['url' => 'pengelola', 'slug' => 'pengelola', 'btn' => 'Pengelola'])
            @include('components.search-input', ['url' => 'pengelola', 'text' => 'pengelola'])
        </div>
    </div>
    ...
    <div class="w-full overflow-hidden rounded-lg shadow-xs">
        ...
        <table class="w-full whitespace-no-wrap">
            ...
            >
            @foreach ($pengelolas as $key => $pengelola)
            <tr class="text-gray-700 dark:text-gray-400">
                <td class="px-4 py-3">
                    {{ $pengelolas->firstItem() + $key }}
                </td>
        ...
        <span class="flex items-center col-span-3">
            Showing {{ $pengelolas->firstItem() }} - {{ $pengelolas->lastItem() }} of {{ $pengelolas->total() }}
        </span>
        <span class="col-span-2"></span>
        <!-- Pagination -->
        <span class="flex col-span-4 mt-2 sm:mt-auto sm:justify-end">
            <nav aria-label="Table navigation">
                {{ $pengelolas->links('pagination::tailwind') }}
            </nav>
        </span>
        </div>
